...
#############################################################
# step 1, procedure 2: merge DAG structures by intersection #
#############################################################

bnXYZ <- empty.graph(c(common, specificA, specificB))
arcs(bnXYZ) <- combi(whiteA, whiteB, "intersection")$all.arcs
...
match.fit_intersection <- custom.fit(bnXYZ, dist = dist)

save(match.fit_intersection, file="../data/match.fit_intersection.rda")
...
for(i in 1:nrow(matchA)){
  set.seed(10+(100*i))
  matchA$doctor[i] <- sample(levels(datB$doctor),1,prob=match.fit_intersection$doctor$prob[,which(levels(datB$sex)==matchA$sex[i])])
  matchA$hospital[i] <- sample(levels(datB$hospital),1,prob=match.fit_intersection$hospital$prob[,which(levels(datB$sex)==matchA$sex[i])
  ...
  matchA$alcohol[i] <- sample(levels(datB$alcohol),1,prob=match.fit_intersection$alcohol$prob[,which(levels(datB$sex)==matchA$sex[i])
                                                                                              ,which(levels(datB$marital.status)==matchA$marital.status[i])
                                                                                              ,which(levels(datB$hospital)==matchA$hospital[i])
                                                                                              ])
  matchA$smoke[i] <- sample(levels(datB$smoke),1,prob=match.fit_intersection$smoke$prob[,which(levels(datB$sex)==matchA$sex[i])
                                                                                        ,which(levels(datB$hospital)==matchA$hospital[i])
                                                                                        ])
  matchA$health.state[i] <- sample(levels(datB$health.state),1,prob=match.fit_intersection$health.state$prob[,which(levels(datB$sex)==matchA$sex[i])
                                                                                                             ,which(levels(datB$doctor)==matchA$doctor[i])
                                                                                                             ,which(levels(datB$hospital)==matchA$hospital[i])
                                                                                                             ])
}
...
for(i in 1:nrow(matchB)){
  set.seed(10+(100*i))
  matchB$pray[i] <- sample(levels(datA$pray),1,prob=match.fit_intersection$pray$prob)
  matchB$confession[i] <- sample(levels(datA$confession),1,prob=match.fit_intersection$confession$prob[,which(levels(datA$sex)==matchB$sex[i])
                                                                                                       ,which(levels(datA$graduation)==matchB$graduation[i])
                                                                                                       ,which(levels(datA$pray)==matchB$pray[i])
                                                                                                       ])
  matchB$church[i] <- sample(levels(datA$church),1,prob=match.fit_intersection$church$prob[,which(levels(datA$confession)==matchB$confession[i])])
  matchB$exp.god[i] <- sample(levels(datA$exp.god),1,prob=match.fit_intersection$exp.god$prob[,which(levels(datA$sex)==matchB$sex[i])
                                                                                              ,which(levels(datA$confession)==matchB$confession[i])
                                                                                              ,which(levels(datA$church)==matchB$church[i])
                                                                                              ,which(levels(datA$pray)==matchB$pray[i])
                                                                                              ])
  matchB$exp.supernat[i] <- sample(levels(datA$exp.supernat),1,prob=match.fit_intersection$exp.supernat$prob[,which(levels(datA$church)==matchB$church[i])
                                                                                                             ,which(levels(datA$exp.god)==matchB$exp.god[i])
                                                                                                             ])
  matchB$faith.healer[i] <- sample(levels(datA$faith.healer),1,prob=match.fit_intersection$faith.healer$prob[,which(levels(datA$sex)==matchB$sex[i])
                                                                                                             ,which(levels(datA$graduation)==matchB$graduation[i])
...
match.dat.intersection_post <- rbind(matchA[,c(common, specificA, specificB)], matchB[,c(common, specificA, specificB)])
match.dat.intersection_post$doctor <- factor(match.dat.intersection_post$doctor, levels=levels(datB$doctor))
match.dat.intersection_post$hospital <- factor(match.dat.intersection_post$hospital, levels=levels(datB$hospital))
match.dat.intersection_post$smoke <- factor(match.dat.intersection_post$smoke, levels=levels(datB$smoke))
match.dat.intersection_post$alcohol <- factor(match.dat.intersection_post$alcohol, levels=levels(datB$alcohol))
match.dat.intersection_post$health.state <- factor(match.dat.intersection_post$health.state, levels=levels(datB$health.state))

save(match.dat.intersection_post, file="../data/match.dat.intersection_post.rda")
